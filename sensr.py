...
def fair_dist(proj_compl):
    ...
    return lambda x, y: torch.sum(torch.square(torch.matmul(x-y,proj_compl_)), dim=1)
...
def sample_perturbation(model, x, y, sensetive_directions, regularizer = 100, learning_rate = 5e-2, num_steps = 200):
    x_start = x.clone()
    x.requires_grad=True
    x_start.requires_grad=False
    sensetive_directions_ = normalize_sensitive_directions(sensetive_directions)
    for i in range(num_steps):
        prob = model(x)
        perturb = unprotected_direction(x-x_start, sensetive_directions_)
        loss = nn.CrossEntropyLoss(y, prob)  - regularizer  * torch.norm(perturb)**2
        gradient = torch.autograd.gradient(loss, x)
